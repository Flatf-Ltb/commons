...
public class Percentile {
    ...
    /**
     * @return boolean - true if sufficient samples have been seen to form an
     * estimate
     */
     ...
     *
     * @return float[]
     * @throws InsufficientSamplesException - if no estimate is currently available
    ...
    private void addMeasurement(final float x) {
        ...
        for (int i = 2; i <= 2 * m + 2; i++) {
            ...
            if ((dval >= 1F) && (dp > 1F)) {
                ...
                final float qt = q[i] + ((1F - dm) * qp + (dp - 1F) * qm) / (dp - dm);
                ...
                final float qt = q[i] - ((1F + dp) * qm - (dm + 1F) * qp) / (dp - dm);
                if ((q[i - 1] < qt) && (qt < q[i + 1])) {
                    q[i] = qt;
                } else {
                    q[i] = q[i] - qm;
